...
namespace TravelaFinalApp.Persistence.Implementations
{
    public class CategoryService(ICategoryRepository categoryRepository, IMapper _mapper) : ICategoryService
    {
        ...
        {
            if (await categoryRepository.IsExist(c => c.Name.ToLower().Trim() == categoryCreateDto.Name.ToLower().Trim()))
                throw new CustomException(400, "Name", $"{categoryCreateDto.Name} category already exist..");
